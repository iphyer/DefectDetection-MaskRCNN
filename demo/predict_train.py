...
if __name__ == "__main__":    
    ...
    for image_name in os.listdir(testPath):
        ...
        image = cv2.imread("../datasets/train/" + image_name)
        #imshow(image)
        # prepare gt mask
        catIds = coco.getCatIds()
        imgIds = coco.getImgIds(catIds=catIds )
        gt_labels = list()
        with open('../datasets/annotations/train.json') as json_data:
        ...
        for i in range(len(anns)):
            gt_numbers_singleImage[gt_labels[i] - 1] += 1 
            if gt_labels[i] == 1:
