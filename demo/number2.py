...
if __name__ == "__main__":    
    ...
    for image_name in os.listdir(testPath):
        ...
        classWrong = np.zeros(4)
        gt_numbers_singleImage = np.zeros(4)
        pred_numbers_singleImage = np.zeros(4)
        gt_mask_list = [[],[],[],[]]
        #print(img)
        ...
        print("f1",F1_list)
        print("location error:", singleLocationErr)
        print("wrong classes list:", classWrong)
    ...
    for i in range(4):
        ...
        recall_avg[i] = recall_avg[i]/10
        prec_avg[i] = prec_avg[i]/10
        F1_avg[i] = F1_avg[i] / 10
    print(total_recall)
    print(total_prec)
    print(total_F1)
